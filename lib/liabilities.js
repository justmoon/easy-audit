...
var Liabilities = function (data, config) {
  if ("undefined" === typeof data.accounts) {
    throw new Error("Invalid liabilities data, no 'accounts' property.");
  }
  if (!Array.isArray(data.accounts)) {
    throw new Error("Invalid liabilities data, 'accounts' is not an array.");
  }
...
Liabilities.prototype.buildTree = function () {
  ...
  if (this._config.verbose) {
    this._config.logTo.write("Building tree with "+this._accounts.length+" accounts\n");
  }
...
Liabilities.fromFile = function (path, config) {
  ...
  var liabilities = new Liabilities(data, config);
  return liabilities;
};
